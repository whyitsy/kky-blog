...
**✅ CI 状态**：见 [GitHub Actions](https://github.com/whyitsy/kky-blog/actions) —— 四个并行检查作业（格式卫生 / 前端类型检查与构建 / 后端编译测试覆盖率 / 依赖漏洞扫描），通过后构建并发布应用镜像；**上线由版本 tag 触发**（`git push origin vX.Y.Z`），部署后会从公网核对线上版本与本次提交一致。合并进 `main` 只发布镜像、不动线上
